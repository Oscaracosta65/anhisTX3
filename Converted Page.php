 ...
 * Montana Big Sky TX3 (Pick 4 + Bonus Ball)
 * Joomla 5.x + PHP 8.1+
 *
...
if (isset($stateName, $gName) && $stateName !== '' && $gName !== '') {
    $doc->setTitle(
        ...
        ' — Results & Frequency Analysis | LottoExpert.net'
    );
    $doc->setMetaData(
        'description',
        'View the latest ' . (string) $stateName . ' ' . (string) $gName .
        ' draw results, number frequency analysis, active and quiet numbers, and draw history. Analytical intelligence by LottoExpert.net.'
    );
}
...
/**
 * --------------------------------------------------------------------------
 * Game configuration — TX3 specific
 * --------------------------------------------------------------------------
 */
$gameMainCount  = 4;
$gameMainMin    = 1;
...
$gameBonusMin   = 1;
$gameBonusMax   = 16;
$gameMainCols   = ['first', 'second', 'third', 'fourth'];
$gameBonusCol   = 'fifth';
...
function leFetchRecentDrawsTX3(
...
function leGetPreviousOccurrenceDateTX3(
    \Joomla\Database\DatabaseDriver $db,
    string $dbCol,
    string $gameId,
    string $drawDate,
...
function leGetDrawingsSinceDateTX3(
    \Joomla\Database\DatabaseDriver $db,
    string $dbCol,
    string $gameId,
    ?string $previousDate,
 ...
 * --------------------------------------------------------------------------
 */
$latestRows = leFetchRecentDrawsTX3($db, (string) $dbCol, $gameId, 1);
...
$rowsMain  = leFetchRecentDrawsTX3($db, (string) $dbCol, $gameId, $nodCurrentMain);
$rowsBonus = ($nodCurrentBonus === $nodCurrentMain)
    ? $rowsMain
    : leFetchRecentDrawsTX3($db, (string) $dbCol, $gameId, $nodCurrentBonus);
...
foreach ($rowsBonus as $idx => $row) {
    $ball = trim((string) ($row[$gameBonusCol] ?? ''));
...
$window50  = leFetchRecentDrawsTX3($db, (string) $dbCol, $gameId, 50);
$window300 = leFetchRecentDrawsTX3($db, (string) $dbCol, $gameId, 300);
...
$top50  = leTopKeysByValue($counts50,  $gameMainCount, false);
$top300 = leTopKeysByValue($counts300, $gameMainCount, false);
...
if ($drawDate !== '') {
    foreach ($latestMainBalls as $ball) {
        $prevDate = leGetPreviousOccurrenceDateTX3($db, (string) $dbCol, $gameId, $drawDate, $ball, false, $gameMainCols, $gameBonusCol);
        $drawsAgo = leGetDrawingsSinceDateTX3($db, (string) $dbCol, $gameId, $prevDate, $drawDate);
    ...
    if ($pb !== '') {
        $prevBonusDate = leGetPreviousOccurrenceDateTX3($db, (string) $dbCol, $gameId, $drawDate, $pb, true, $gameMainCols, $gameBonusCol);
        $bonusDrawsAgo = leGetDrawingsSinceDateTX3($db, (string) $dbCol, $gameId, $prevBonusDate, $drawDate);

        $drawHistoryRows[] = [
            'label'    => htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8') . ' (' . lePad2($pb) . ')',
 ...
 * --------------------------------------------------------------------------
 */
$jsonLdPageName = isset($stateName, $gName)
    ? ((string) $stateName . ' ' . (string) $gName . ' Results and Frequency Analysis')
    : 'Lottery Results and Frequency Analysis';

$jsonLdPageDesc = isset($stateName, $gName)
    ? ('Latest ' . (string) $stateName . ' ' . (string) $gName . ' results, number frequency, active and quiet numbers, and historical draw analysis.')
    : 'Lottery frequency and results analysis.';
      ...
      <div class="skai-hero-top">
        ...
        <div class="skai-hero-copy">
          ...
          <div class="skai-hero-actions" aria-label="Primary actions">
            <a class="skai-btn skai-btn--primary" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=TX3">
              Open SKAI Analysis
            </a>
            <a class="skai-btn skai-btn--secondary" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=TX3">
              AI Predictions
            </a>
          ...
          <div class="skai-advanced-links" aria-label="Advanced tools">
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=TX3">Skip &amp; Hit Analysis</a>
            <a class="skai-mini-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=TX3">MCMC Markov Analysis</a>
            <a class="skai-mini-link" href="/all-lottery-heatmaps?gameId=TX3">Heatmap Analysis</a>
          </div>
        </div>
  ...
  <section class="skai-strip" aria-label="Key takeaways">
    ...
    <article class="skai-stat">
      ...
      <div class="skai-stat-body">
        ...
        <div class="skai-stat-note">Main-number and <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> draw windows currently loaded for this page view.</div>
      </div>
    </article>
  </section>
    ...
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Start with a clear high-level view. This layer is designed for fast orientation: which numbers are most active, which are quiet, and how the current draw relates to recent history. The <?php echo htmlspecialchars((string) $gName, ENT_QUOTES, 'UTF-8'); ?> draws <?php echo (int) $gameMainCount; ?> main numbers from a pool of <?php echo (int) $gameMainMax; ?> plus one <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> from a pool of <?php echo (int) $gameBonusMax; ?>.
        </p>
      </div>
    </div>
          ...
          <div class="skai-card-body">
            <div class="skai-chart-shell">
              <div class="skai-chart-frame">
                <canvas id="topActiveChart" aria-label="Top active main numbers chart" role="img"></canvas>
              </div>
            </div>
          </div>
        </div>
              ...
              <?php foreach ($drawHistoryRows as $row) : ?>
                <div class="skai-history-item">
                  <div class="skai-history-name">
                    <?php if (!empty($row['isBonus'])) : ?>
                      <span style="display:inline-flex;align-items:center;gap:6px;">
                        <span class="skai-pill skai-pill--bonus" style="width:28px;height:28px;font-size:12px;"><?php echo htmlspecialchars(lePad2((string) $pb), ENT_QUOTES, 'UTF-8'); ?></span>
                        <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?>
                      </span>
                    <?php else : ?>
  ...
  <section id="frequency-deep-dive" class="skai-section" aria-labelledby="frequency-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Move from summary to full reference. The first panel shows the complete main-number distribution across all values 01&ndash;<?php echo str_pad($gameMainMax, 2, '0', STR_PAD_LEFT); ?>. The second panel shows the complete <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution for the selected bonus window. Frequency counts how many times each number appeared in the selected draw window.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            ...
            <span class="skai-card-sub">All values 01&ndash;<?php echo str_pad($gameMainMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentMain; ?> drawings</span>
          </div>
          <div class="skai-card-body">
            <div class="skai-chart-shell">
        ...
        <div class="skai-grid">
          <div class="skai-card">
            <div class="skai-card-head skai-card-head--radiant">
              ...
              <span class="skai-card-sub">All values 01&ndash;<?php echo str_pad($gameBonusMax, 2, '0', STR_PAD_LEFT); ?> across the last <?php echo (int) $nodCurrentBonus; ?> drawings</span>
            </div>
            <div class="skai-card-body">
              <div class="skai-chart-shell">
                <div class="skai-chart-frame">
                  <canvas id="bonusChart" aria-label="<?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> distribution chart" role="img"></canvas>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="skai-note">
        The complete distribution is best used as a reference layer. The overview modules above are optimized for quick reading; this section is optimized for thorough review. Recency measures how many draws have passed since a number last appeared &mdash; a higher value means the number has been absent for longer.
      </div>
    </div>
  </section>
  ...
  <section id="tables" class="skai-section" aria-labelledby="tables-title">
    <div class="skai-section-head">
      <div>
        ...
        <p class="skai-section-sub">
          Use the tables for exact counts, recency, and personal tracking. Quick filters help narrow the view without losing full access to the complete dataset. The main-number table covers values 01&ndash;<?php echo str_pad($gameMainMax, 2, '0', STR_PAD_LEFT); ?>. The <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> table covers values 01&ndash;<?php echo str_pad($gameBonusMax, 2, '0', STR_PAD_LEFT); ?>.
        </p>
      </div>
    </div>
    ...
    <div class="skai-section-body">
      <div class="skai-two-col">
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--horizon">
            Main numbers table
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo str_pad($gameMainMax, 2, '0', STR_PAD_LEFT); ?></span>
          </div>
        ...
        <div class="skai-card">
          <div class="skai-card-head skai-card-head--radiant">
            ...
            <span class="skai-card-sub">Exact counts and recency for values 01&ndash;<?php echo str_pad($gameBonusMax, 2, '0', STR_PAD_LEFT); ?></span>
          </div>

          <div class="skai-table-wrap">
            <table id="skai-bonus-table" class="skai-table" aria-label="<?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> frequency table">
              <thead>
                <tr>
                  <th>Number</th>
                  <th>Drawn Times</th>
      ...
      <div class="skai-tool-grid">
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skai-lottery-prediction?gameId=TX3">Open SKAI Analysis</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/ai-powered-predictions?game_id=TX3">Open AI Predictions</a>
          </div>
        </article>
        ...
        <article class="skai-tool">
          ...
          <div class="skai-tool-body">
            ...
            <a class="skai-tool-cta" href="/picking-winning-numbers/artificial-intelligence/skip-and-hit-analysis?game_id=TX3">Open Skip &amp; Hit</a>
          </div>
        </article>
      </div>

      <div class="skai-utility-grid">
        <a class="skai-utility-link" href="/picking-winning-numbers/artificial-intelligence/markov-chain-monte-carlo-mcmc-analysis?gameId=TX3">MCMC Markov Analysis</a>
        <a class="skai-utility-link" href="/all-lottery-heatmaps?gameId=TX3">Heatmap Analysis</a>
  ...
  <section class="skai-section" aria-labelledby="method-title">
    ...
    <div class="skai-section-body">
      <div class="skai-method-note">
        <strong>Interpretation guidance:</strong> Frequency, recency, and spacing can provide useful context for reviewing draw history, but they should be treated as descriptive signals rather than guarantees. <strong>Active numbers</strong> are those that appeared most often in the selected window. <strong>Quiet numbers</strong> are those whose last appearance was furthest in the past within the same window. The <?php echo htmlspecialchars($gameBonusLabel, ENT_QUOTES, 'UTF-8'); ?> pool (01&ndash;<?php echo str_pad($gameBonusMax, 2, '0', STR_PAD_LEFT); ?>) is tracked separately and never combined with main-number counts. The purpose of this page is to make the recent behavior of the game easier to understand, compare, and carry into deeper SKAI analysis.
      </div>
    </div>
  </section>
...
(function () {
  ...
  function safeCanvas(id) {
    ...
    if (parent && parent.offsetWidth > 0) { return el; }
    return el;
  }
